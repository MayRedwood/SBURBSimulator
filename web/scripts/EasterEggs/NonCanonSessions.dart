...
abstract class NonCanonSessions {
    ...
    static Map<int, dynamic> get sessionMap {
        ...
        ret[518225815] = generalSession; //huh?
        //RMH wants 117
        //gnome wants 85
    ...
    //As death ensures near, 6 players miss.
    static void generalSession(Session session) {
        int numPlayers = 6;
        makeASessionFromSource(session,generalSessionIndexToPlayer, numPlayers);
        session.players.length = numPlayers; //no more, no less.
        Player fae  = session.players[0];
        Player cd = session.players[1];
        Player rd = session.players[2];
        Player ic = session.players[3];
        Player oc = session.players[4];
        Player wl = session.players[5];
    }
   ...
   static void session730(Session session){
    ...
    static void generalSessionIndexToPlayer(Session session,Player player, int index){
        Session s = session;
        player.deriveChatHandle = false;
        player.deriveLand = false;
        if(index == 0){

            player.copyFromOCDataString("b=%C3%B3%C3%B2%C2%8D%17%C3%BE%02%00Z%22%22%2B&s=,,Satire,Fine Art,fae&x=AQ==");
            player.deriveSprite = false;
            player.deriveSpecibus = false;
            player.fraymotifs.clear();
            player.deriveSpecibus = false;
            ...
            player.copyFromOCDataString("b=%C2%91h%3CB%C3%BE%C2%88%00%13%26%16%07&s=,,Crochet,Fashion,rogueofDonuts&x=AQ==");
            player.deriveSprite = false;
            player.deriveSpecibus = false;
            player.fraymotifs.clear();
            player.specibus = new Specibus("2x3dent", ItemTraitFactory.TRIDENT, [ ItemTraitFactory.METAL, ItemTraitFactory.BLUNT]);
            ...
            player.copyFromOCDataString("b=%00%00%00N%C3%BE%C2%9A%00U--%1F&s=,,Movies,Programming,originalCathexis&x=pnAA");
            player.deriveSprite = false;
            player.deriveSpecibus = false;
            player.fraymotifs.clear();
            player.specibus = new Specibus("Longbow", ItemTraitFactory.BOW, [ ItemTraitFactory.WOOD, ItemTraitFactory.POINTY]);
            ...
            player.quirk.suffix = "";
            player.land = player.spawnLand();
            player.land.name = "Land of Games and Confusion";
            player.land.denizenFeature = new DenizenFeature('Ārohirohi');

        }else if(index == 4){

            player.copyFromOCDataString("b=%C3%BF%00%00%11%C3%B6V%00%18%06%0E%0F&s=,,Genocide,Social Justice,ideaticContainment&x=jggA");
            player.deriveSprite = false;
            player.deriveSpecibus = false;
            player.fraymotifs.clear();
            player.specibus = new Specibus("Crossbow", ItemTraitFactory.BOW, [ ItemTraitFactory.METAL, ItemTraitFactory.POINTY]);
            ...
            player.quirk.suffix = "";
            player.land = player.spawnLand();
            player.land.name = "Land of Stroms and Mightmares";
            player.land.denizenFeature = new DenizenFeature('Awilix');

        }else if(index == 5){

            player.copyFromOCDataString("b=%00%00%00%C3%87%C3%BE%2B%00Q-%24%04&s=,,Fine Art,Religion,wizardlyLogger&x=AQ==");
            player.deriveSprite = false;
            player.deriveSpecibus = false;
            player.fraymotifs.clear();
            player.specibus = new Specibus("Dagger", ItemTraitFactory.DAGGER, [ ItemTraitFactory.METAL, ItemTraitFactory.EDGED]);
            ...
            player.quirk.suffix = "";
            player.land = player.spawnLand();
            player.land.name = "Land of Water and Eons";
            player.land.denizenFeature = new DenizenFeature('Minerva');
